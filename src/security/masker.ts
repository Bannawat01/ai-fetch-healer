...
export class Masker implements PayloadMasker {
  ...
  constructor(options: MaskerOptions = {}) {
    ...
    this.sensitiveKeys = new Set(
      [...DEFAULT_SENSITIVE_KEYS, ...additional].map((key) => normalizeKey(key))
    );
  ...
  private getSensitiveMask(key: string): JsonValue {
    ...
    if (normalized.includes('username') || normalized.includes('user')) return 'masked_identity';
    if (normalized.includes('password')) return 'masked_password';
    if (normalized.includes('token') || normalized.includes('apikey') || normalized.includes('api')) {
      return 'masked_token';
    }
