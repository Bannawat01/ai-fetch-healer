...
function isHealingRule(value: unknown): value is HealingRule {
  ...
  const validMapping = candidate.mapping === undefined || isStringMap(candidate.mapping);
...
export function createHealedFetch(provider: ILLMProvider, config: HealerConfig = {}) {
  ...
  return async function healedFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    try {
      ...
      if (!Number.isFinite(contentLength) || contentLength <= maxErrorDetailsChars * 4) {
      ...
      if (!healingRule) {
        console.log(`[ai-fetch-healer] Cache miss. Consulting AI (${provider.name})...`);
        const healResult = await provider.heal(maskedSchema, errorDetails);
        healingRule = healResult.rule;

        if (!isHealingRule(healingRule)) {
          console.warn('[ai-fetch-healer] Invalid healing rule from provider. Returning original response.');
          return ensureReadableResponse(response);
        }
        
        cache.set(cacheKey, healingRule);
      } else {
        console.log('[ai-fetch-healer] Cache hit! Applying healing rule locally.');
      }
      ...
      if (healingRule.action !== 'MAP_FIELDS') {
        console.warn(`[ai-fetch-healer] Unsupported action "${healingRule.action}". Applying mapping fallback.`);
      }
      ...
      console.warn('[ai-fetch-healer] Auto-healing failed, returning original response.', error);
      return ensureReadableResponse(response);
    }
  };
}